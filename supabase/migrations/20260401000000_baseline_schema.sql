-- =============================================================================
-- Baseline schema captured from LIVE Supabase introspection on 2026-04-21
-- (see audit/live_db_verification_2026-04-21.md for the evidence pack;
-- cross-referenced against audit/live_schema/tables.md + src/services +
-- src/data/adapters.js). Pre-dates the migration history. Sort-order: this
-- file (20260401) applies before all 20260421_* migrations from Track C.
--
...
-- ---------------------------------------------------------------------------
-- therapy_cycles  (live shape verified 2026-04-21)
-- Sources: live Supabase introspection (primary) + src/data/adapters.js +
-- src/services/dataService.js + docs/MON_ARCHITECTURE_DONNEES.md.
-- ⚠ user_id FK is NO ACTION live (not CASCADE as §04 inferred).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- invoices  (live shape verified 2026-04-21 — 7-column skeleton)
-- ⚠ Confirmed at the DB level that this table has no invoice_number,
--   no amount, no TVA fields, no status. §06 R-05 (L441-9 non-compliance)
--   is a structural gap, not just a code omission.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- invoice_sessions  (live shape verified 2026-04-21)
-- Live has BOTH a composite PK on (invoice_id, session_id) AND a UNIQUE on
-- session_id alone — a session can belong to at most ONE invoice ever.
-- Stricter than §04's H-7 assumed; flag to product if the domain rule
-- actually permits re-invoicing a session under a corrective invoice.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- clients — 11 drifted columns (live-verified 2026-04-21)
-- Note: docs/MON_ARCHITECTURE_DONNEES.md mentions four additional columns
-- (axes_travail, points_vigilance, objectifs, dynamique_relationnelle) that
-- do NOT exist in the live DB. They are doc-only ghosts and must NOT be
-- added here — the correct fix is to patch the doc (tracked as a Track F
...
--  dynamique_relationnelle removed 2026-04-21 — not present in live DB
--  per audit/live_db_verification_2026-04-21.md §3.5. See header comment.)
...
-- No triggers, no user-defined functions in any non-system schema.
-- Verified 2026-04-21 via:
--   SELECT p.proname FROM pg_proc p JOIN pg_namespace n ON p.pronamespace=n.oid
--     WHERE n.nspname NOT IN ('pg_catalog','information_schema','pg_toast',
--       'storage','auth','realtime','extensions','graphql','graphql_public',
--       'vault','pgbouncer','supabase_functions','net','pgsodium',
--       'pgsodium_masks','cron') AND p.prokind='f'        → [] (0 rows)
--   SELECT tgname FROM pg_trigger WHERE NOT tgisinternal  → [] (0 rows)
--
-- clients.updated_at and professionals.updated_at are maintained by application
-- code (dataService.updateClient, dataService.updateProfessional) — no DB trigger.
-- Storage buckets: zero (verified). Nothing to declare here.
