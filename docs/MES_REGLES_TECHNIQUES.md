...
- **Passage d'objets aux Helpers** : Les fonctions d'aide à la détection de type ou d'état (ex: `getClientType`, `getComputedStatus`) doivent impérativement recevoir l'objet métier complet (ex: `client`) et non seulement son ID. Cela permet de vérifier les propriétés internes (partnerB, status, etc.) sans requêtes supplémentaires.
- **Priorité au Calcul Dynamique (UI)** : Pour les indicateurs de progression (ex: nombre de séances réalisées), privilégier systématiquement un calcul dynamique dans l'UI via `useMemo` plutôt que de se fier à des colonnes de compteurs en base de données qui peuvent se désynchroniser.
